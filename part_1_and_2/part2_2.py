...
query_sudoku = torch.load("query_sudoku.pth")
target_sudoku = torch.load("target_sudoku.pth")
class sudokuDataset(Dataset):
  def __init__(self, tensors):

    self.data_size = len(query_sudoku)
    self.query = query_sudoku.reshape(self.data_size,64).long()
    self.answer = target_sudoku.reshape(self.data_size,64).long() - 1
...
class RRN(nn.Module):
  def __init__(self,emb_dim=16,num_steps=25,mlp_dim=96,hidden_dim=16):
   ...
   # positions = torch.tensor([[(i, j) for i in range(8) for j in range(8)] for b in range(self.batch_size)])
    ...
    self.output = nn.Linear(in_features = hidden_dim, out_features = 9) #9!
  ...
  def forward(self,sudoku,answers):
    ...
    for steps in range(self.num_steps):
      ...
      l = nn.CrossEntropyLoss()(out.reshape(bsz*64,9),answers.reshape(bsz*64))
      loss+=l
      # assert 1==2
      # print(l)
        ...
        if(clock<patience):
          ...
          for phase in ['train', 'val','test']:
              ...
              for i,(sudoku,solved) in tqdm(enumerate(dataloaders[phase])):
                  ...
                  digit_corrects += ((preds)==so.data).sum()
                  digit_total += len(so)*64
                  corrects = ((preds)==so.data).sum(dim=1) == 64
...
test_query_sudoku = torch.load('test_query.pth')
# test_query_sudoku
...
for i,sudoku in tqdm(enumerate(test_query_sudoku)):
  ...
  _, preds = torch.max(logits, 2)
  preds = preds.detach().cpu().reshape(64)
