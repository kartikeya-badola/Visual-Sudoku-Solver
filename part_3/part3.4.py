...
folders = ([name for name in os.listdir('gan_Dataset/')
            if os.path.isdir(os.path.join('gan_Dataset/', name))])
for folder in folders:
  contents = os.listdir(os.path.join('gan_Dataset/', folder))
...
for folder in folders:
  ...
  for filename in contents:
    file_path = os.path.join(os.path.join('gan_Dataset/', folder), filename)
    try:
  ...
  print('New Size: ' + str(len(os.listdir(os.path.join('gan_Dataset/', folder)))))



transform = transforms.Compose([transforms.Grayscale(),
        transforms.ToTensor()])
dataset = datasets.ImageFolder('gan_Dataset',transform)
...
def sample_image(g,n_classes, hidden_dim,name):
    ...
    # Sample noise
    z = torch.normal(0,1,(n_classes**2,hidden_dim)).to(device)
    ...
    torchvision.utils.save_image(gen_imgs.data, "images/"+name+".png", nrow=n_classes, normalize=True)
...
train_model(g,d,Goptim,Doptim,Gscheduler,Dscheduler,n_classes=9,hidden_dim=100,num_epochs=7,patience=7)
...
target_list = []
for i in range(9):
    folder = os.listdir('gan_Dataset/'+str(i))
    for j in range(1000):
        path = 'gan_Dataset/'+str(i)+'/'+folder[j]
